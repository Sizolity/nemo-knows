# nemo-knows Cloudflare 部署（可选）

CF Worker + Tunnel 是 nemo-web 的可选部署方式，当前生产环境不使用。

## 当前生产架构

```
腾讯云域名 → Shingo (Astro 静态站，对外)
Shingo → localhost:8788 → nemo-server (后端 API，内部)
```

- **nemo-server** 是唯一的生产后端服务，监听 localhost:8788
- **shingo** 是对外的前端站点
- **nemo-web** 保留在仓库中作为本地开发/可选部署方式，不上线

## 可选：通过 CF Worker 暴露 nemo-web（仅开发/演示用）

以下仅在需要远程访问 nemo-web 控制台时使用。
...
# 配置隧道（仅暴露 nemo-web，nemo-server 保持 localhost）
mkdir -p ~/.cloudflared
...
ingress:
  - hostname: nemo-origin.shingo.dpdns.org
    service: http://localhost:8787
  - service: http_status:404
EOF

# 添加 DNS 记录
cloudflared tunnel route dns nemo nemo-origin.shingo.dpdns.org
...
nemo-server 监听 localhost:8788，不通过 CF Tunnel 或 Worker 暴露。
Shingo 等同机服务通过 localhost 直接调用：

```
curl http://127.0.0.1:8788/api/wiki/pages
curl -X POST -H "X-Webhook-Token: $TOKEN" http://127.0.0.1:8788/hooks/content
```
...
- 给 shingo 站也加 Worker 做边缘缓存（静态站天然适合）
