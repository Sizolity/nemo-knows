...
`[[ingest]]` turns raw material into source, concept, entity, or topic pages. `[[query]]` answers from the existing wiki and can save valuable answers back into it. `[[lint]]` periodically checks the wiki for contradictions, missing links, stale claims, and gaps.

## Why It Matters

The wiki is a `[[wiki-as-compounding-artifact]]`: every source and useful question can improve the shared knowledge structure instead of disappearing into chat history.
